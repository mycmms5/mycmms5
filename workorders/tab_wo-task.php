...
* 
* CVS
* $Id: tab_wo-task.php,v 1.7 2013/11/04 07:50:04 werner Exp $
* $Source: /var/www/cvs/mycmms40/mycmms40/workorders/tab_wo-task.php,v $
* $Log: tab_wo-task.php,v $
* Revision 1.7  2013/11/04 07:50:04  werner
* CVS version shows
*
* Revision 1.6  2013/09/27 09:14:41  werner
...
public function page_content() {
    ...
    $tpl->assign('tasks',$DB->query("SELECT task.TASKNUM AS 'id',task.DESCRIPTION AS 'text' FROM task 
        LEFT JOIN taskeq ON task.TASKNUM=taskeq.TASKNUM
        WHERE taskeq.EQNUM='{$this->input2}'",PDO::FETCH_NUM));
    $tpl->display_error("tab_wo-task.tpl");
...
*/
        ...
        if (is_null($e)) {
            $_SESSION['Message_Success'] = "WO ".$_REQUEST['WONUM']." succesfully saved into TASK".$_REQUEST['TASKNUM'];
        }
        break;
        case "TASK2WO":        
            try {
            ...
            if (is_null($e)) {
                $_SESSION['Message_Success'] = "TASK ".$_REQUEST['TASKNUM']." succesfully copied to WO".$_REQUEST['WONUM'];
            }
            break;
        case "WO2WO":
            try {
...
*/
                if ($_REQUEST['NEWWO']=='on') {
                    ...
                    array("eqnum"=>$_REQUEST['EQNUM'],"taskdesc"=>$taskdesc,"texts_a"=>$texts_a,"originator"=>$_SESSION['user'],"approvedby"=>$_SESSION['APPROVER']));
...
*/
            ...
            if (is_null($e)) {
                $_SESSION['Message_Success'] = "WO ".$_REQUEST['WONUM']." succesfully copied to WO".$param_WONUM_NEW;
            }
            break;            
        } // EO switch
} // EO process_form
