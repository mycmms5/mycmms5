...
/**
* TODO: Add a pre-defined operation here
*/
            $DB->commit();        
            return __FILE__." OK";
        } catch (Exception $e) {
...
**/        
    ...
    if ($_REQUEST['DIRECTPLAN']=="on") {
        try {
            $DB->beginTransaction();
            DBC::execute("INSERT INTO woop (WONUM,OPNUM,OPDESC,OPSCHEDULE) VALUES (:wonum,10,'EXECUTION',:schedstartdate)",array("wonum"=>$_REQUEST['id1'],"schedstartdate"=>$_REQUEST['SCHEDSTART']));
            DBC::execute("INSERT INTO wocraft (WONUM,CRAFT,OPNUM,TEAM,ESTHRS) VALUES (:wonum,'MECH',10,1,8)",array("wonum"=>$_REQUEST['id1']));
            $DB->commit();        
            return __FILE__." OK";
        } catch (Exception $e) {
            $DB->rollBack();
