...
function process_form() {
    ...
    try {
        ...
        if ($_REQUEST['VENDORID']!=$_REQUEST['VENDOR_OLD']) { // New Primary Vendor
            ...
            DBC::execute("INSERT INTO invvend (ITEMNUM,VENDORID,PRIMARYVENDOR,MANUFACTID,UNITCOST,UNITQTY,UOP,MINORDERQTY) VALUES (:itemnum,:vendorid,'Y',:manufactid,:unitcost,:unitqty,:uop,:minorderqty)",array("itemnum"=>$this->input1,"vendorid"=>$_REQUEST['VENDORID'],"manufactid"=>$_REQUEST['MANUFACTID'],"unitcost"=>$_REQUEST['UNITCOST'],"unitqty"=>$_REQUEST['UNITQTY'],"uop"=>$_REQUEST['UOP'],"minorderqty"=>$_REQUEST['MINORDERQTY']));
...
?>
<tr><td class="LABEL"><?PHP echo create_radio("vendor_switch",$row->VENDORID,"checked")." ".$row->VENDORID;; ?></td><td><?PHP echo _("UnitCost")." : ".$row->UNITCOST; ?></td><td><?PHP $row->MANUFACTID; ?></td></tr>    
<?PHP
        } else {
?>
<tr><td class="LABEL"><?PHP echo create_radio("vendor_switch",$row->VENDORID,"")." ".$row->VENDORID; ?></td><td><?PHP echo _("UnitCost")." : ".$row->UNITCOST; ?></td><td><?PHP $row->MANUFACTID; ?></td></tr>    
<?PHP
        }
    }    
}
