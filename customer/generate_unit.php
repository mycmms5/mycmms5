...
*/
        $result=$DB->query("SELECT EQNUM,EQROOT,postid,parent,unit FROM $Table WHERE unit IS NULL OR unit=''");
        foreach ($result->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $unit=DBC::fetchcolumn("SELECT unit FROM equip WHERE postid='{$row['parent']}'",0);
            if (is_null($unit) OR ($unit=='')) {
                $no_cc++;
            } else {
#                DebugBreak();
                echo "<tr><td>".$row['EQNUM']."</td><td>".$row['EQROOT']."</td><td>".$CC."</td></tr>";
                DBC::execute("UPDATE $Table SET unit=:unit WHERE EQNUM=:eqnum",array("unit"=>$unit,"eqnum"=>$row['EQNUM']));    
            }
        } 
...
*/
        ...
        $tpl->assign("title","Look for a UNIT in the parent(s)");
