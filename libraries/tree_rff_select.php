...
* CVS
* $Id: tree_rff_select.php,v 1.2 2013/04/17 06:04:44 werner Exp $
* $Source: /var/www/cvs/mycmms40/mycmms40/libraries/tree_rff_select.php,v $
* $Log: tree_rff_select.php,v $
...
*/
	private function getChildren($parentCode){
		$query = sprintf("SELECT * FROM rff_tree WHERE parent='%s'", mysql_escape_string($parentCode));
		...
		while ($line = mysql_fetch_assoc($result)) {
			...
			$node = DBTreeView::createTreeNode(
			    ...
			    $node->setURL(sprintf("javascript:
                    opener.document.treeform.RFF.value='$rff';
                    opener.document.treeform.RFF_DESCRIPTION.value='$description';
                    window.close(); "));
		        //has children
			    $query2 = sprintf("SELECT * FROM rff_tree WHERE parent='%s' LIMIT 1", mysql_escape_string($code));
