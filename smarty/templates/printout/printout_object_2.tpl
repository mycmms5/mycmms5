...
{* spares *}
<table>
<tr><th>{t}DBFLD_ITEMNUM{/t}<BR>MAPICS</th>
    <th>{t}DBFLD_DESCRIPTION{/t}</th>
    <th>{t}DBFLD_NOTES{/t}</th>
    <th>{t}WAREHOUSE{/t}</th>
    <th>{t}DBFLD_LOCATION{/t}</th>
    <th>{t}DBFLD_QTYONHAND{/t}</th></tr>
{foreach item=spare from=$spares}
<tr><td>{$spare.ITEMNUM}/<b>{$spare.MAPICS}</b></td>
    <td>{$spare.DESCRIPTION}</td>
    <td>{$spare.NOTES}</td>
    <td>{$spare.WAREHOUSEID}</td>
...
<tr><th>CLMCHID</th>
    <th>CLID</th>
    <th>Name</th>
    <th>Machine description</th></tr>
{foreach item=install from=$installed}
<tr><td>{$install.EQNUM}</td>
    <td>{$install.EQROOT}</td>
    <td>{$install.NAME}</td>
    <td>{$install.DESCRIPTION}</td></tr>
{/foreach}
