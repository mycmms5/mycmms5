...
<input type="hidden" name="ID" value="{$prestation.ID}">
    ...
    <td>{$prestation.WODATE|date_format:"%Y-%m-%d"}</td>
...
<input type="hidden" name="form_save" value="SET">
<input type="hidden" name="ID" value="{$prestation.ID}">
    <td>{include file="_combobox.tpl" type="LIST"  options=$employees NAME="EMPCODE" SELECTEDITEM=$prestation.EMPCODE}</td>
    <td>{$prestation.OPNUM}</td>
    <td>{include file="_calendar2.tpl" NAME="WODATE" VALUE=$prestation.WODATE|date_format:"%Y-%m-%d"}</td>
...
<tr bgcolor="{cycle values="WHITE,LIGHTSTEELBLUE"}">
    ...
    <td>{$prestation.OPNUM}</td>
    <td>{include file="_calendar2.tpl" NAME="WODATE" VALUE=$smarty.now|date_format:"%Y-%m-%d"}</td>
