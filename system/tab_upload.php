...
* @filesource
* CVS
* $Id: tab_upload.php,v 1.3 2013/11/04 08:03:08 werner Exp $
* $Source: /var/www/cvs/mycmms40/mycmms40/system/tab_upload.php,v $
* $Log: tab_upload.php,v $
* Revision 1.3  2013/11/04 08:03:08  werner
* BUG: show only linked files no uploads
*
...
*/
    ...
    move_uploaded_file($file_info['tmp_name'],"../documents/upload/tmp/".$file_info['name']);
/**
* Smarty
...
*/
    ...
    $tpl->display_error("upload_db_check.tpl");
    break;
} // EO Step 1
case "2": {
/** 
...
*/
    ...
    try {
        ...
        switch ($_REQUEST['ACTION']) {
            ...
            $upload_errors=array();
            $upload_errors['mkdir']=mkdir("../documents/upload/".$stored,NULL,TRUE);
            $upload_errors['move']=rename("../documents/upload/tmp/".$_REQUEST['filename'],"../documents/upload/".$stored."/".$_REQUEST['filename']);
            $upload_errors['exists']=file_exists("../documents/upload/".$stored."/".$_REQUEST['filename']);
            break;                            
        }
        $DB->commit();
    } catch (Exception $e) {
    ...
    $tpl->debugging=false;
    $tpl->assign("stylesheet",STYLE_PATH."/".CSS_SMARTY);
    $tpl->assign("md5",$_REQUEST['md5']);
    $tpl->assign("sha1",$_REQUEST['sha1']);
    $tpl->assign("store_path",$stored."/".$file_info['name']);
    $tpl->assign("upload_errors",$upload_errors);
    $tpl->display_error("upload_storage_point.tpl");
    break;
} // EO Step 2
default: {
    ...
    $tpl->display_error("upload_form.tpl");
} // EO default
}
?>
