...
public function page_content() {
    ...
    $tpl->display_error('tab_tskcraft_allfields.tpl');    
} // End page_content
    ...
    case "UPDATE":
        ...
        try {
            ...
            if (strlen($_REQUEST['OPDESC'])>5 AND $_REQUEST['ESTHRS']!="0") {
                $values=DBC::fetchcolumns("SELECT TASKNUM,OPNUM FROM tskcraft WHERE ID={$_REQUEST['ID']}");
                DBC::execute("UPDATE tskop SET EQNUM=:eqnum,OPDESC=:opdesc,OPLDESC=:opldesc WHERE TASKNUM=:tasknum AND OPNUM=:op",array("eqnum"=>$_REQUEST['EQNUM'],"opdesc"=>$_REQUEST['OPDESC'],"opldesc"=>$_REQUEST['OPLDESC'],"tasknum"=>$values[0],"op"=>$values[1]));
            }
            $DB->commit();
        } catch (Exception $e) {
            $DB->rollBack();
