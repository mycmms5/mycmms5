<!--
This module shows an object - its children, parts, failure causes and maintenance actions

The objective is to iterate through the different databases:
- tbl_sysPart       Key spt_ID / spt_spt_ID (0 = root)
- tbl_sysComponent  Key scm_ID / scm_spt_ID (link to sysPart)
- tbl_sysFailMode   Key sfm_ID / sfm_spt_ID (link to sysPart) / sfm_Desc
- tbl_sysFailCause  Key sfc_ID  sfc_scm_ID (link to sysComponent) sfc_sfm (link to sysFailMode)
-tbl_sysMaintenance Key smn_ID  smn_sfc_ID (link to sysFailCause) smn_spt_ID (link to sysPart) / smn_description Maintenance Beurten zijn gelinkt naar PART
-->
<html>
<head>
<title>OPTIMIZER+ access</title>
</head>
<body>
<h1 align='center'>OPTIMIZER+</h1>
<h5 align='center'>This application iterates through a selection of Systems.</h5><br/>
<?php
$serverName = "XPS\SQLEXPRESS";
/* Connect using Windows Authentication. */
try {
    $conn = new PDO("sqlsrv:server=$serverName ; Database=OPTIMIZER01", "sa", "sdeygh");
...
/* Get AdventureWorks products by querying against the product name.*/
    case 'getproducts':
        try {
            ...
            $tsql = "SELECT spt_ID, spt_Tag, spt_Name, spt_Function FROM dbo.tbl_sysPart WHERE spt_Tag LIKE '%' + ? + '%'";
...
/* Upload a new picture for the selected product. */
    case 'uploadpicture':         
    try {
        $tsql = "INSERT INTO Production.ProductPhoto (LargePhoto) VALUES (?)";
...
echo "<table align='center'>
<form action='optimizer_demo.php' 
  enctype='multipart/form-data' method='POST'>
